...
@is_teller
def transaction_statement(request, teller=None):
    account_number = request.GET.get(
        "account_number")  # Get the account number from query parameters
    ...
    if account_number:
        transactions = transactions.filter(account_number=account_number)
